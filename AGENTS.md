...
- `Web → Application → Domain`（Web **不得**依赖 `cli`）
- `Application → Presentation`；`Application → API/Infra`（经 `Session`）
- `API/Infra → Models`
- **禁止反向依赖**：
  - `domain` / `models` 不得依赖 `api`、`cli`、`web`、`application`、`presentation`
...
| 层 | 目录 | 允许 | 禁止 |
|----|------|------|------|
| 入口 | `cli/`、`web/` | Clap/Axum 定义、参数映射 → Request | 业务逻辑、HTTP 解析、分析公式 |
...
**入口独占**（仅 `main`、`cli::run`、`web::run`、`config::load` 允许）：
...
Web 构建启用 Leptos 时会额外触发一批既有函数的 `cognitive_complexity` 告警；`--features web` 的 clippy 暂以 `-A clippy::cognitive_complexity` 放行，待后续拆分相关函数后收紧。
...
5. Web 与 CLI 是否复用同一 `application` 用例，而非在 `web/` 重写 IO 逻辑？
...
- 在 `web/` 或 `presentation/` 内构造 HTTP client、读写缓存（应经 `Session`）
