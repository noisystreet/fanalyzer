...
use crate::schema::responses::{
    ...
    FetchSuccessEnvelope, HoldingsSuccessEnvelope, InfoSuccessEnvelope, PortfolioSuccessEnvelope,
    RankSuccessEnvelope, ScreenSuccessEnvelope, SectorsSuccessEnvelope, SUCCESS_ENVELOPES,
};
...
fn write_index(output_dir: &Path) -> anyhow::Result<()> {
    ...
    let index = json!({
        ...
        "usage": {
            "tools": "MCP / function-calling 入参 schema（自 Clap 生成）",
