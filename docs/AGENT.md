...
| `INVALID_ARGS` | 缺参、参数不合法 |
| `UNKNOWN_TOOL` | MCP 工具名不存在 |
| `NO_OUTPUT` | 命令成功但未捕获到 JSON（内部异常） |
| `INVALID_OUTPUT` | MCP 工具返回非 JSON |
...
| 字段 | 说明 |
|------|------|
| `content[0].text` | JSON 信封字符串（与 CLI `json` 子命令 stdout 同构） |
| `structuredContent` | 同一信封的 JSON **对象**（声明了 `outputSchema` 时必填） |
| `isError` | 业务失败时为 `true`（与信封 `ok: false` 对齐）；成功时省略 |

失败时 `structuredContent` / `content` 均为统一失败信封（见 `schemas/envelope.failure.json`），含 `error.code` / `message` / 可选 `retryable` / `hint`。客户端在 `isError: true` 时应跳过对成功 `outputSchema` 的校验，改为按失败信封解析。

## Agent 调用建议

1. **始终解析** CLI stdout JSON，或 MCP 的 `structuredContent`（优先）/`content[0].text`
...
6. 失败时查看 `error.hint` 与 `error.retryable` 决定是否重试；MCP 上同时看 `isError`
